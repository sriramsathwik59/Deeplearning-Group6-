...
class PredNet(nn.Module):
    ...
    def forward(self, x):
      print(f"Input shape in forward method: {x.shape}")
      batch_size, num_frames, channels, height, width = x.size()
      x = x.reshape(batch_size, num_frames * channels, height, width)
      print(f"Input shape after reshape in forward method: {x.shape}")

      x = self.encoder(x)
      print(f"Output shape after encoder: {x.shape}")
      x = self.decoder(x)
      print(f"Output shape after decoder: {x.shape}")

      x = x.reshape(batch_size, num_output_frames, channels, height, width)
      print(f"Output shape after final reshape in forward method: {x.shape}")
      return x
...
def predict_frames(model, input_frames):
    print(f"Input frames shape before unsqueeze: {input_frames.shape}")
    input_frames = input_frames.squeeze(0)  # Remove the extra dimension
    print(f"Input frames shape after squeeze: {input_frames.shape}")
    
    with torch.no_grad():
        predicted_frames = model(input_frames.unsqueeze(0))  # Add batch dimension
        print(f"Predicted frames shape: {predicted_frames.shape}")
        predicted_frames = predicted_frames.squeeze(0).permute(0, 2, 3, 1).cpu().numpy()
        predicted_frames = (predicted_frames * 255).astype(np.uint8)
    return predicted_frames[0]  # Return only the first predicted frame

def create_gif(frames, output_path):
    imageio.mimsave(output_path, frames, duration=0.1)

# Load the trained model
model = PredNet((num_input_frames, img_size[0], img_size[1], num_channels)).to(device)
model.load_state_dict(torch.load("prednet_model_set00_set01.pth"))
model.eval()
...
# Predict frames for each test sequence
# Predict frames for each test sequence
processed_sequences = set()  # Keep track of processed sequence folders
...
for data in test_dataloader:
    ...
    if sequence_folder not in processed_sequences:
        print(f"Input frames shape from dataloader: {input_frames.shape}")
        
        # Predict the frames
        predicted_frames = predict_frames(model, input_frames)
        
        # Squeeze the extra dimensions from predicted_frames
        predicted_frames = predicted_frames.squeeze()
        
        # Create a GIF of the predicted frames
        gif_output_path = os.path.join(sequence_folder, "predicted_frames.gif")
        create_gif([predicted_frames], gif_output_path)
        print(f"GIF created for sequence: {sequence_folder}")
